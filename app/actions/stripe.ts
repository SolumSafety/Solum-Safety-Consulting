...
export async function startCheckout(
  ...
  try {
    const session = await stripe.checkout.sessions.create({
      mode: "payment",
      line_items: [
        // Products with a real, pre-registered Stripe price (e.g. the Tier 2
        // assessment) MUST reference it by price ID. A price_data line item
        // generates a brand-new, one-off Price ID on every single checkout —
        // for a product that needs to be recognised again after payment
        // (like the assessor app matching a purchase back to a tier), that
        // dynamic price can never match anything. This was the root cause of
        // Tier 2 purchases never granting assessment access.
