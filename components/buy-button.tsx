...
export function BuyButton({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {/* Promo code */}
      {promoApplied ? (
        <div className="flex items-center justify-between gap-2 rounded-lg border border-terracotta/30 bg-terracotta/10 px-3 py-2">
          <span className="flex min-w-0 items-center gap-2 text-xs font-medium text-foreground">
            <Check className="h-3.5 w-3.5 shrink-0 text-terracotta" aria-hidden="true" />
        ...
        <div>
          ...
          {promoError && <p className="mt-1.5 text-xs font-medium text-terracotta">{promoError}</p>}
        </div>
      ) : (
        <button
          type="button"
